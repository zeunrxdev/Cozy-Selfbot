...
current_emoji = ""
autoreplies = [
"hey am afk ill see ur msg when am back"
]
...
{www}║            xblood Selfbot              ║
{www}╠════════════════════════════════════════╣
{www}║ Made by: {mjj}zenmuder7.          {www}║
{www}║ Welcome: {mjj}{bot.user}          {www}║
{www}╚════════════════════════════════════════╝
"""
TOKEN="MTMzOTI2MDY2Njc3NDgxODg1Mw.GF8xnr.ofE7vk_CcI_q5LoNZbZINrQbW2_k8bsg7p1bZ8"
if not TOKEN:
...
{www}║                ^-^Selfbot                     ║
{www}╠════════════════════════════════════════╣
{www}║ Made by: {mjj}hauntedsouls.7             {www}║
{www}║ Welcome: {mjj}{bot.user}                 {www}║
{www}╚════════════════════════════════════════╝
""")
pages = [
...
async def help(ctx, page: int = 1):
    ...
    if 1 <= page <= total_pages:
        await ctx.send(f"```Welcome to [ ^-^ alk selfbot ] ```")
        message = await ctx.send(pages[page - 1] + f"```ansi\n              {www}Page {page}/{total_pages} - Type {red}p#{www} to navigate.                 Welcome [ {black}{bot.user}{www} ^-^]        Made by [ @hauntedsouls7. ]         ```")
...
async def menu(ctx):
    ...
    loading_message = await ctx.send("```please enter the password to access the menu. 💀```")

    try:
        msg = await bot.wait_for('message', timeout=30.0, check=check)

        if msg.content != "catutu":
            await loading_message.edit(content="```incorrect password. Access denied. 💀```")
    ...
    "⠀⠀⠀⠀⠀⠀   ⢀⣠⠤⠶⠒⠒⠛⠛⠓⠒⠶⠀⠀⠀⠀⠀⠀",
        ...
        for line in loading_lines:
            ...
            await loading_message.edit(content=f"```{text}```")

        await asyncio.sleep(1)

        menu_main = """``
 
            {www}[1] {red}autoreacting{ww}
            {www}[2] {red}streaming status{www}
``"""

        page_1 = """```
  {www}{red}auto reacting{www}
{www}- {red}autoreact @user emoji{www}
{www}- {red}autoreactoff @user{www}

``"""

        await loading_message.edit(content=f"```{menu_main}```")

        def page_check(m):
            return m.author == ctx.author and m.content in ["main","1","menustop"]

        while True:
            try:
                msg = await bot.wait_for('message', timeout=30.0, check=page_check)
                ...
                if msg.content == "main":
                    ...
                    await loading_message.edit(content=f"```{page_1}```")
                    await msg.delete()

                elif msg.content == "menu stop":
                     await loading_message.delete()
                     await msg.delete()  
                     break
        ...
        await ctx.send("```You took too long to enter the password. Please try again. 💀```")
